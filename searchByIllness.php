  ...
  text-align: left;
    ...
    <a style = "text-align:right" class="navbar-brand" href="Admin_Log_In.php">Admin Login</a>
		...
		<form name= "search" action = "searchByMed.php" method = "POST">
			<h1 align="center"> Result Found</h1>
			<h4 style = "text-align:center; color: gray">Search By Illness</h4><br>
		</form>
		...
		// Create connection
			...
			$name = $_POST["illness_name"];
	
			$sql = "select medicine.medicine_id,medicine.name as medicine,brand.name as brand from medicine join brand on medicine.medicine_id = brand.brand_id join medicine_illness on medicine.medicine_id = medicine_illness.medicine_id join illness on medicine_illness.illness_id = illness.illness_id where illness.name like '%$name%'";
